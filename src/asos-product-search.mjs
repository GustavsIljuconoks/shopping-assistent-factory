...
import { ASOS_RETAILER_IDENTIFIER } from "./asos-login-flow.mjs";

export { ASOS_RETAILER_IDENTIFIER };
export const ASOS_SEARCH_URL_BASE = "https://www.asos.com/search/";
export const ASOS_PRODUCT_SEARCH_MAX_CANDIDATES = 3;
...
export async function searchAsosProducts({
  intent,
  products,
  searchProducts,
  browserRun,
  ...
  chat,
  profilesRoot,
  searchUrlBase = ASOS_SEARCH_URL_BASE,
  maxCandidates = ASOS_PRODUCT_SEARCH_MAX_CANDIDATES,
  memories = [],
} = {}) {
  const normalizedIntent = normalizeAsosProductIntent(intent);
  ...
  const searchUrl = createAsosSearchUrl(normalizedIntent, searchUrlBase);
  const rawProducts =
    products ??
    (await fetchAsosProducts({
      browserRun,
      chat,
  ...
  if (!Array.isArray(rawProducts)) {
    throw new TypeError("ASOS product search adapter must return an array.");
  }

  return rankAsosProducts(rawProducts, normalizedIntent, rankingMemories)
    .slice(0, normalizedMaxCandidates)
    .map(({ candidate }) => candidate);
}

export function createAsosSearchUrl(intent, searchUrlBase = ASOS_SEARCH_URL_BASE) {
  const normalizedIntent = normalizeAsosProductIntent(intent);
  const url = new URL(searchUrlBase);
  url.searchParams.set("q", buildAsosSearchQuery(normalizedIntent));
  return url.toString();
}

export function normalizeAsosProductIntent(intent) {
  if (!intent || typeof intent !== "object" || Array.isArray(intent)) {
    throw new TypeError("ASOS product intent must be an object.");
  }
...
function buildAsosSearchQuery(intent) {
  ...
  try {
    const result =
      typeof searchProducts === "function"
        ...
        : await extractAsosProductCards(session);
...
async function extractAsosProductCards(session) {
  if (session && typeof session.extractProducts === "function") {
    return session.extractProducts({ retailerIdentifier: ASOS_RETAILER_IDENTIFIER });
  }
  if (session && typeof session.evaluate === "function") {
    return session.evaluate(() =>
      Array.from(
        document.querySelectorAll(
          "article[data-auto-id='productTile'], article, [data-testid*='product'], [class*='product-card']",
        ),
      )
        .map((card) => {
  ...
  throw new TypeError("session must provide extractProducts or evaluate for ASOS search.");
}

function rankAsosProducts(products, intent, memories = []) {
  return products
    .map((product, index) => normalizeAsosProductCandidate(product, intent, index))
...
function normalizeAsosProductCandidate(product, intent, index) {
  if (!product || typeof product !== "object" || Array.isArray(product)) {
    throw new TypeError("ASOS product candidates must be objects.");
  }
...
function normalizePrice(value, field) {
  ...
  if (typeof value === "string") {
    ...
    return {
      amount,
      currency: value.includes("£") || value.toUpperCase().includes("GBP") ? "GBP" : value.toUpperCase().includes("USD") ? "USD" : "EUR",
    };
  }
