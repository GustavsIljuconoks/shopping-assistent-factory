...
import {
  ...
  searchAsosProducts,
} from "../src/asos-product-search.mjs";
...
test("returns up to 3 ranked ASOS product candidates for a structured intent", async () => {
  const results = await searchAsosProducts({
    intent: SHIRT_INTENT,
    products: [
  ...
  assert.equal(results[0].brand, "ASOS");
  assert.equal(results[0].size, "M");
  assert.match(results[0].reasoning, /Matches shirt, white\/off white, size M under EUR 120\./);
});

test("filters ASOS candidates above the price ceiling and mismatched size or color", async () => {
  const results = await searchAsosProducts({
    intent: SHIRT_INTENT,
    products: [
...
test("parses ASOS price text during candidate extraction", async () => {
  const results = await searchAsosProducts({
    intent: SHIRT_INTENT,
    products: [
      product({
...
test("de-prioritizes ASOS candidates that match negative Shopping memories", async () => {
  const results = await searchAsosProducts({
    intent: SHIRT_INTENT,
    memories: [
      {
...
test("uses a search adapter with ASOS search URL and normalized intent", async () => {
  const calls = [];
  ...
  assert.equal(calls[0].retailerIdentifier, ASOS_RETAILER_IDENTIFIER);
  ...
  assert.equal(calls[0].searchUrl, "https://www.asos.com/search/?q=shirt+white");
  assert.equal(results.length, 1);
});

test("launches the visible browser with ASOS's persistent profile when a launcher is provided", async () => {
  ...
  try {
    const results = await searchAsosProducts({
      ...
      searchProducts: async ({ session: callbackSession, searchUrl }) => {
        ...
        assert.equal(searchUrl, "https://www.asos.com/search/?q=shirt+white+off+white");
        return [product({ title: "Oxford Shirt", price: { amount: 95, currency: "EUR" }, sizes: ["M"] })];
      },
    });
    ...
    assert.equal(events[0][1].retailer, "ASOS");
    assert.equal(events[0][1].startUrl, "https://www.asos.com/search/?q=shirt+white+off+white");
...
test("extracts ASOS candidates from the browser session when no adapter is provided", async () => {
  const profilesRoot = await mkProfileRoot();
  const session = {
...
function product(overrides = {}) {
  return {
    productUrl: "https://www.asos.com/selected-femme-oxford-shirt-white-se521e1ab-a11.html",
...
async function mkProfileRoot() {
  return mkdtemp(join(tmpdir(), "asos-product-search-"));
}
