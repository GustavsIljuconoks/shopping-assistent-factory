...
import {
  ...
  searchAboutYouProducts,
} from "../src/aboutyou-product-search.mjs";
...
test("returns up to 3 ranked About You product candidates for a structured intent", async () => {
  const results = await searchAboutYouProducts({
    intent: SHIRT_INTENT,
    products: [
  ...
  assert.equal(results[0].brand, "About You");
  assert.equal(results[0].size, "M");
  assert.match(results[0].reasoning, /Matches shirt, white\/off white, size M under EUR 120\./);
});

test("filters About You candidates above the price ceiling and mismatched size or color", async () => {
  const results = await searchAboutYouProducts({
    intent: SHIRT_INTENT,
    products: [
...
test("parses About You price text during candidate extraction", async () => {
  const results = await searchAboutYouProducts({
    intent: SHIRT_INTENT,
    products: [
      product({
...
test("de-prioritizes About You candidates that match negative Shopping memories", async () => {
  const results = await searchAboutYouProducts({
    intent: SHIRT_INTENT,
    memories: [
      {
...
test("uses a search adapter with About You search URL and normalized intent", async () => {
  const calls = [];
  ...
  assert.equal(calls[0].retailerIdentifier, ABOUT_YOU_RETAILER_IDENTIFIER);
  ...
  assert.equal(calls[0].searchUrl, "https://www.aboutyou.lv/c/sievietem-20201?q=shirt+white");
  assert.equal(results.length, 1);
});

test("launches the visible browser with About You's persistent profile when a launcher is provided", async () => {
  ...
  try {
    const results = await searchAboutYouProducts({
      ...
      searchProducts: async ({ session: callbackSession, searchUrl }) => {
        ...
        assert.equal(searchUrl, "https://www.aboutyou.lv/c/sievietem-20201?q=shirt+white+off+white");
        return [product({ title: "Oxford Shirt", price: { amount: 95, currency: "EUR" }, sizes: ["M"] })];
      },
    });
    ...
    assert.equal(events[0][1].retailer, "About You");
    assert.equal(events[0][1].startUrl, "https://www.aboutyou.lv/c/sievietem-20201?q=shirt+white+off+white");
...
test("extracts About You candidates from the browser session when no adapter is provided", async () => {
  const profilesRoot = await mkProfileRoot();
  const session = {
...
function product(overrides = {}) {
  return {
    productUrl: "https://www.aboutyou.lv/only-white-shirt-example-p123456.html",
    imageUrl: "https://cdn.aboutyou.com/example-shirt.jpg",
    title: "Oxford Shirt",
    color: "White",
...
async function mkProfileRoot() {
  return mkdtemp(join(tmpdir(), "aboutyou-product-search-"));
}
